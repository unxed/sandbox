...
4. Ветки `vmlab-cmd`/`vmlab-out` рассчитаны на **одну сессию**: вторая сессия при старте
   делает force-push в `vmlab-out` и затирает результаты первой. `ctl.py` теперь ждёт, пока
   `SESSION` в `vmlab-out` станет равен id своего запуска (иначе читает результаты
   предыдущей сессии).
...
* Общая сессия для нескольких агентов: сейчас одна на репозиторий (см. грабли №4);
  решение — путь результатов с id сессии вместо одной ветки `vmlab-out`.
