...
* **`f4 --gui=x11` в Haiku сразу выходит с кодом 0 и ничего не показывает** (X-сервера нет; окна
  нет, сообщения нет). Для Haiku нужен нативный бэкенд на app_server, либо хотя бы понятное
  сообщение и откат в терминальный режим — отдельная задача.
