...
sftp, wazero, archives, f4 — включая новый `pty_haiku.go` и
`internal/gui` allow-листы, tar) поверх тулчейна `unxed/go` (плюс новые
коммиты с `FcntlInt`/`Getpgid` в самом форке). Дальше — по тому же
принципу: реальная ошибка компиляции → патч (в `patches/`, не напрямую
в чужие репозитории; для самого форка компилятора — коммит в
`unxed/go`, т.к. это уже наш репозиторий) → коммит → новый прогон.
