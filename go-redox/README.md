...
Снимок на 2026-09-19, голова ветки `ed01d4ecb12d7db834ebda22d908d52e3d633406` (72 коммита, 157 файлов).
...
- Обновляется автоматически: workflow `go-redox-snapshot` (раз в 3 часа и по запросу) пересобирает файлы из ветки
  и коммитит сюда, если ветка изменилась; в начале этого README `Снимок` тогда не обновляется, актуальная голова записана в `HEAD.txt`.

Связанное в этом репозитории: `relibc-sigentry-rcx/` и `kernel-forcekill-lost-wakeup/` (патчи для GitLab в формате `git am`),
`f4-redox/` (f4 на Redox), `vm-ci/RECIPE.md` и `vmlab/` (быстрый цикл с виртуалкой и скриншотами).
